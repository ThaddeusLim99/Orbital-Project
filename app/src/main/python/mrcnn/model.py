...
#enabled to allow for tflite conversion
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    try:
        print("trying timedistributed")
        x = KL.TimeDistributed(KL.Conv2D(fc_layers_size, (pool_size, pool_size), padding="valid"),
                               name="mrcnn_class_conv1")(x)
    except:
        print("timedistributed error")
        return
...
class MaskRCNN(object):
    ...
    def __init__(self, mode, config, model_dir):
        ...
        self.set_log_dir()
        try:
            print("trying self.build")
            self.keras_model = self.build(mode=mode, config=config)
        except:
            print("self.build error")
            return
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            try:
                print("try fpn_classifier_graph")
                ...
                print("fpn_classifier_graph error")
                return
            # Detections
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        with h5py.File(filepath, mode='r') as f:
            ...
            if by_name:
                try:
                    print("trying load weights")
                    hdf5_format.load_weights_from_hdf5_group_by_name(f, layers)
                except:
                    print("loading weights error")
                    return
            else:
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            try:
                print("trying utils.resize_image")
                ...
                print("utils.resize_image error")
                return
    ...
    def detect(self, images, verbose=0):
        ...
        if verbose:
            ...
            for image in images:
                log("image", image)
        print("before mold_input")
        # Mold inputs to format expected by the neural network
        try:
            print("try molded_inputs")
            molded_images, image_metas, windows = self.mold_inputs(images)
        except:
            print("molded_inputs error")
            return
        print("after mold_input")
        ...
        print("before get_anchors")
        # Anchors
        try:
            print("trying get_anchors")
            anchors = self.get_anchors(image_shape)
        except:
            print("get_anchors error")
            return
        # Duplicate across the batch dimension because Keras requires it
        # TODO: can this be optimized to avoid duplicating the anchors?
        try:
            print("trying broadcast_to")
            anchors = np.broadcast_to(anchors, (self.config.BATCH_SIZE,) + anchors.shape)
        except:
            print("broadcast_to error")
            return
        if verbose:
        ...
        try:
            ...
            return
        # Process detections
        print("before process detections")
        results = []
        try:
            print("trying unmold_detections")
